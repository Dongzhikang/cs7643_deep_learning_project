...
def main():
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        avg_train_acc_history.append(acc.cpu().detach().item())
        avg_train_loss_history.append(loss)

        # eval for one epoch
        val_loss, val_acc = validate(val_loader, model, classifier, criterion, opt)
        
        avg_val_acc_history.append(val_acc.cpu().detach().item())
        # print("val acc is ", val_acc.cpu().detach().numpy())
    ...
    print('train acc is', avg_train_acc_history)
    print('val acc is', avg_val_acc_history)
    ...
    outfile=open('./save/results_data/val_acc'+opt.model_name+'.csv','w')
    writer=csv.writer(outfile)
    writer.writerow(avg_val_acc_history)

    outfile=open('./save/results_data/train_acc'+opt.model_name+'.csv','w')
    writer=csv.writer(outfile)
    writer.writerow(avg_train_acc_history)
